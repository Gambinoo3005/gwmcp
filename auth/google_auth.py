...
if _client_secrets_env:
    ...
    # Check user home directory first (more likely for uvx/pip installs)
    _home_cs_path = os.path.join(
        os.path.expanduser("~"), ".google_workspace_mcp", "client_secret.json"
    )
    if os.path.exists(_home_cs_path):
        CONFIG_CLIENT_SECRETS_PATH = _home_cs_path
    else:
        # Fallback: assumes this file is in auth/ and client_secret.json is in the root
